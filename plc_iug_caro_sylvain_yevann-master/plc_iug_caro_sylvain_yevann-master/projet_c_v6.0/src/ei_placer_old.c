...
void ei_placer_run(struct ei_widget_t* widget)
{
        ...
        ei_size_t widget_size = widget->requested_size;
        ...
        if (widget->placer_params->rw != NULL) {
                widget_size.width+= (int) (((float) parent_rect.size.width)*widget->placer_params->rw_data);
        }
        if (widget->placer_params->rh != NULL) {
                widget_size.height+= (int) (((float) parent_rect.size.height)*widget->placer_params->rh_data);
        }
        ...
        switch ((ei_anchor_t) widget_anchor) {
                case ei_anc_northwest:
                        break;
                case ei_anc_center:
                        widget->screen_location.top_left.x+= parent_rect.size.width / 2;
                        widget->screen_location.top_left.y+= parent_rect.size.height / 2;
                        break;
                case ei_anc_north:
                        widget->screen_location.top_left.x+= parent_rect.size.width / 2;
                        break;
                case ei_anc_south:
                        widget->screen_location.top_left.x+= parent_rect.size.width / 2;
                        widget->screen_location.top_left.y+= parent_rect.size.height;
                        break;
                case ei_anc_east:
                        widget->screen_location.top_left.x+= parent_rect.size.width;
                        widget->screen_location.top_left.y+= parent_rect.size.height / 2;
                        break;
